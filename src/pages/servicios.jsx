        ...
        <div className="w-full min-h-screen overflow-x-hidden bg-black relative">
            ...
            <section className="bg-radial-dark text-acento py-20 px-5 sm:px-6 md:px-10" id="servicios">
                <div className="max-w-7xl mx-auto text-center mb-16">
                    <motion.h2 className="text-3xl sm:text-3xl md:text-4xl righteous text-acento font-bold mb-4" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                        <FormattedMessage id="servicios.cards.titulo" />
                    </motion.h2>
                    <motion.p className="text-acento inter text-base sm:text-lg md:text-2xl max-w-2xl mx-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 md:gap-8 max-w-7xl mx-auto">
                    {servicios.map((servicio, index) => (
                        <motion.div key={index} className="bg-zinc-900 p-4 sm:p-5 rounded-2xl shadow-lg shadow-cyan-500 hover:shadow-2xl transition-shadow cursor-pointer" whileHover={{ scale: 1.05 }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: index * 0.2 }}>
                            <div className="text-2xl mb-2 text-cyan-500 flex justify-center">{servicio.icon}</div>
                            <h3 className="text-lg sm:text-lg md:text-lg inter text-center text-cyan-500 font-semibold mb-2">{servicio.title}</h3>
                            <p className="text-acento merri text-center text-sm sm:text-base md:text-base">{servicio.description}</p>
                        </motion.div>
                    ))}
                </div>
            </section>
            ...
            <div className="bg-radial-dark py-20">
                <div className="container mx-auto px-5 sm:px-6 md:px-10 flex flex-col md:flex-row items-start gap-6 md:gap-12">
                    <motion.div className="md:w-1/2 relative flex-shrink-0" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
                        <motion.img src="imgs/servicios/responsive.jpg" alt="Cómo consigo mi web" className="w-full h-auto rounded-lg shadow-md relative z-10" animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 4, ease: "easeInOut" }} />
                    ...
                    <motion.div className="md:w-1/2" initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
                        <h1 className="text-3xl md:text-3xl righteous font-bold text-acento mb-4">
                            <FormattedMessage id="servicios.proceso.titulo" />
                        </h1>
                        <h2 className="text-xl md:text-xl inter text-acento mb-8">
                            <FormattedMessage id="servicios.proceso.subtitulo" />
                        </h2>
                        <ul className="space-y-6">
                            {pasos.map((id, index) => (
                                <motion.li key={index} className="flex items-start" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: index * 0.2 }} viewport={{ once: true }}>
                                    <motion.span className="text-xl md:text-4xl righteous font-bold text-primario flex-shrink-0 mr-6" initial={{ scale: 0 }} whileInView={{ scale: 1 }} transition={{ type: "spring", stiffness: 260, damping: 20, delay: index * 0.2 }} viewport={{ once: true }}>
                                        {index + 1}
                                    </motion.span>
            ...
            <div className="relative">
                {showButtons && (
                    <motion.div
                        ...
                        className="fixed  bottom-20 left-1/2 transform -translate-x-1/2 flex sm:bottom-30  sm:flex-row gap-4 z-[50]"
                    >
                        <a
